...
export default async function SiteGiftsByDomainPage({
  searchParams,
}: {
  ...
  const gifts = list.gifts.filter((gift) => gift.isActive);
  ...
  const giftsWithAvailability = gifts.map((gift) => {
    ...
    return { gift, availableQty, soldOut, displayPrice };
  });
  const visibleGifts =
    ...
    <main className="min-h-screen bg-[#faf7f5]">
      {pageCoverImage ? (
        <div className="relative w-full aspect-[16/9] md:aspect-auto md:h-[78vh]">
          <Image src={pageCoverImage} alt={`Capa da pagina ${pageTitle}`} fill priority sizes="100vw" className="object-cover" />
      ...
      <section className="max-w-6xl mx-auto px-4 py-10">
        {pageMessage ? (
          <p className="mb-8 max-w-3xl whitespace-pre-line break-words" style={{ fontFamily: fontBody, color: messageColor }}>
            {pageMessage}
          </p>
        ) : null}
        {gifts.length === 0 ? (
                ...
                {visibleGifts.map(({ gift, availableQty, soldOut, displayPrice }) => (
                  <article key={gift.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                    ...
                    <div className="p-5">
                      ...
                      {gift.description ? <p className="text-sm text-gray-600 mt-2 min-h-10">{gift.description}</p> : null}
                      <div className="flex items-center justify-between mt-5">
                        ...
                        {soldOut ? (
                          ...
                          <Link
                            href={`/checkout/${gift.id}?slug=${encodeURIComponent(list.slug)}`}
