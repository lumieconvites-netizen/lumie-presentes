...
export default function RsvpConfigPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        if (json.available) {
          setSlugCheck({ status: 'available', message: 'Slug disponível para uso.' });
        } else {
          setSlugCheck({ status: 'taken', message: json?.message || 'Este slug já esta em uso.' });
        }
      } catch (error: any) {
        if (error?.name !== 'AbortError') {
          setSlugCheck({ status: 'error', message: 'Não foi possível validar o slug agora.' });
        }
      }
    }, 350);
  ...
  async function saveSettings(e?: FormEvent) {
    ...
    if (slugCheck.status === 'checking') {
      alert('Aguarde a validação do slug terminar.');
      return;
    }
      ...
      <Card className="border-[#e7d8cb]">
        ...
        <CardContent>
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={saveSettings}>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">Slug da página pública de check-in</label>
              <Input
              ...
              {previewCheckInUrl ? (
                <a href={previewCheckInUrl} target="_blank" rel="noopener noreferrer" className={linkClassName}>
                  Abrir página pública de check-in
                </a>
              ) : null}
            </div>
